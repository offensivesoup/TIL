...
CREATE TABLE new_orders (
    order_id INTEGER PRIMARY KEY,   -- 주문 ID (기본 키)
    order_date DATE,                -- 주문 날짜 (날짜 타입)         -- 총 주문 금액 (실수 타입)
    customer_id INTEGER,
    FOREIGN KEY (customer_id) REFERENCES customers(customer_id)
);
DROP TABLE orders;
ALTER TABLE new_orders RENAME TO orders;
ALTER TABLE orders ADD price INTEGER;

INSERT INTO orders (order_date, price, customer_id) VALUES
    ('2023-07-15', 50, 1),    -- 허균 고객 정보
    ('2023-07-16', 75, 2),  -- 김영희 고객 정보
    ('2023-07-17', 30, 3);    -- 이철수 고객 정보

SELECT order_id, name, order_date, price FROM orders LEFT JOIN customers ON orders.customer_id = customers.customer_id; 
