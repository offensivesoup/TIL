...
def create_user(TmpLst):
    TmpCnt = 0
    for data in user_data:
        if is_validation(data)[0] != True:
            TmpCnt += 1
            pass
        if type(is_validation(data)[0]) == bool:
            if is_validation(data)[0] == False:
                data[is_validation(data)[1][0]] = None
            user_list.append(data)
    
    
    print(f'잘못된 데이터로 구성된 유저의 수는 {TmpCnt} 입니다.')
    print(user_list)
...
def is_validation(data):
    ...
    if len(data['website']) < 1:
        Er_dict['website'] = None
        ErCnt += 1
    if data['company'] in black_list:
        Er_dict['company'] = None
        ErCnt += 1
        return "blocked"
    if ErCnt >= 1:
        return False, [i for i in Er_dict], ErCnt
    elif ErCnt == 0:
        return True, Er_dict, ErCnt
